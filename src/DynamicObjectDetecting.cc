...
std::vector<bbox_t> DynamicObjectDetecting::getSpecifiedObjectBbox(const cv::Mat& mImGray) {
    std::cout << "Start detecting objects!" << std::endl;
    std::vector<bbox_t> results = detector->detect(mImGray, mDetectTh); 
    std::vector<bbox_t> dynaObjs;  
    for (bbox_t bbox : results) {
...
std::vector<bbox_t> DynamicObjectDetecting::getSpecifiedObjectBboxfordrawing(const cv::Mat& mImRGB) {
    std::cout << "Start detecting objects!" << std::endl;
    std::vector<bbox_t> results = detector->detect(mImRGB, mDetectTh); 
    std::vector<bbox_t> dynaObjstodraw;  
    for (bbox_t bbox : results) {
        if (id2name[bbox.obj_id] == "tvmonitor" || id2name[bbox.obj_id] == "mouse" || id2name[bbox.obj_id] == "keyboard" 
        || id2name[bbox.obj_id] == "cup" || id2name[bbox.obj_id] == "teddy bear" || id2name[bbox.obj_id] == "person" 
        || id2name[bbox.obj_id] == "chair") {
            dynaObjstodraw.push_back(bbox); 
        }
	/*if(id2name[bbox.obj_id] == "mouse") {
	    dynaObjstodraw.push_back(bbox); 
	}*/
    }
    return dynaObjstodraw;
}
